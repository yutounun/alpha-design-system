...
- **Deep Navy** (`#061b31`): `--color-foreground-high`. Primary heading color. Not black, not gray -- a very dark blue that adds warmth and depth to text.
...
- **Ruby** (`#ea2261`): `--color-danger`. Warm red-pink for icons, alerts, and accent elements.
- **Magenta** (`#f96bee`): `--color-secondary`. Vivid pink-purple for gradients and decorative highlights.
- **Magenta Light** (`#ffd7ef`): `--color-secondary-muted`. Tinted surface for magenta-themed cards and badges.
...
- **Purple Deep** (`#2e2b8c`): `--color-primary-active`. Dark purple for icon hover states.
- **Purple Light** (`#b9b9f9`): `--color-primary-muted`. Soft lavender for subdued hover backgrounds.
- **Purple Mid** (`#665efd`): `--color-primary`. Range selector and input highlight color.

### Neutral Scale

- **Heading** (`#061b31`): Primary headings, nav text, strong labels.
- **Label** (`#273951`): `--color-foreground-medium`. Form labels, secondary headings.
...
- **Lemon** (`#9b6829`): `--color-warning`. Warning and highlight accent.
...
- Shadow color `rgba(0,55,112,0.08)` (`--shadow-sm-top`) for top-edge shadows on sticky elements
